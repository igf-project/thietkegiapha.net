...
class CLS_PARTNER{
    ...
    public function getListCate($parid=0,$level=0){
        $sql="SELECT partner_id,par_id,name FROM tbl_partner WHERE `par_id`='$parid' AND `isactive`='1' ";
    ...
    public function listTable($strwhere="",$page=1,$parid=0,$level=0,$rowcount){
        ...
        $leng=MAX_ROWS;
        $sql="SELECT * FROM tbl_partner WHERE 1=1 $strwhere AND par_id=$parid ORDER BY `partner_id` DESC LIMIT $star,$leng";
        $objdata=new CLS_MYSQL();
    ...
    public function Add_new(){
        $sql="INSERT INTO `tbl_partner` (`par_id`,`name`,`code`,`intro`,`address`,`phone`,`fax`,`logo`,`website`,`email`,`author`,`isactive`) VALUES ";
    ...
    public function Delete($ids){
        $sql="DELETE FROM `tbl_partner` WHERE `partner_id` in ('$ids')";
        return $this->objmysql->Exec($sql);
    }
    public function setHot($ids){
        $sql="UPDATE `tbl_partner` SET `ishot`=if(`ishot`=1,0,1) WHERE `partner_id` in ('$ids')";
        return $this->objmysql->Exec($sql);
    }
    public function setActive($ids,$status=''){
        $sql="UPDATE `tbl_partner` SET `isactive`='$status' WHERE `partner_id` in ('$ids')";
        if($status=='')
            $sql="UPDATE `tbl_partner` SET `isactive`=if(`isactive`=1,0,1) WHERE `partner_id` in ('$ids')";
        return $this->objmysql->Exec($sql);
    }
    public function Order($arr_id,$arr_quan){
        $n=count($arr_id);
        for($i=0;$i<$n;$i++){
            $sql="UPDATE `tbl_partner` SET `order`='".$arr_quan[$i]."' WHERE `partner_id` = '".$arr_id[$i]."' ";
            $this->objmysql->Exec($sql);
        }
    }
    /* combo box*/
    function getListCbItem($getId='', $swhere='', $arrId=''){
        $sql="SELECT partner_id, name, code FROM tbl_partner WHERE ".$swhere." `isactive`='1' ORDER BY `name` ASC";
        $objdata=new CLS_MYSQL();
        $objdata->Query($sql);
        if($objdata->Num_rows()<=0) return;
        while($rows=$objdata->Fetch_Assoc()){
            ...
            if(!$arrId){
                ?>
                <option value='<?php echo $rows['partner_id'];?>' <?php if(isset($getId) && $id==$getId) echo "selected";?>><?php echo $name;?></option>
                <?php
            }else{?>
