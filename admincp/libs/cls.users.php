...
class CLS_USERS{
	...
	function getMemberByID($memid){
		$sql="SELECT * FROM `tbl_user` WHERE `user_id`=\"$memid\" ";
		$objdata=new CLS_MYSQL();
		$objdata->Query($sql);
		if($objdata->Num_rows()>0){
			$rows=$objdata->Fetch_Assoc();
			$this->pro['ID']=$rows['user_id'];
	...
	public function LOGIN($user,$pass,$type=''){
		...
		if($flag==true){
			...
			$_SESSION[MD5($_SERVER['HTTP_HOST']).'_USERID']=$rows['user_id'];
	...
	function setActive($ids,$status=''){
		$sql="UPDATE `tbl_user` SET `isactive`='$status' WHERE `user_id` in ('$ids')";
		if($status=='')
			$sql="UPDATE `tbl_user` SET `isactive`=if(`isactive`=1,0,1) WHERE `user_id` in ('$ids')";
		return $this->objmysql->Exec($sql);
	}
	function Delete($memid){
		$sql="DELETE FROM `tbl_user` WHERE `user_id` in ('$memid')";
		return $this->objmysql->Query($sql);
	}
}
?>