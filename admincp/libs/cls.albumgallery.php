...
class CLS_ALBUMGALLERY{
	...
	function getThumbByAlbum($album_id){
		...
		$img=$rows['link'];
		if($img!=''){
			$img = strpos($img,'http')!==false?$img:ROOTHOST.PATH_GALLERY_REVIEW.$img;
		}
	...
	function listTable($strwhere="",$page=1){
		...
		$leng=MAX_ROWS;
		$sql="SELECT * FROM `tbl_album` $strwhere ORDER BY `order` ASC LIMIT $star,$leng";
		$objdata=new CLS_MYSQL();
		$objdata->Query($sql);
		$i=0;
		while($rows=$objdata->Fetch_Assoc()){
			...
			if($rows['thumb']!=''){
				$img='<img src="'.stripslashes($rows['thumb']).'" height="45" width="80"/>';
			}else $img=THUMB_DEFAULT;
	...
	function Add_new(){
		$sql=" INSERT INTO `tbl_album`(`name`,`group_id`,`code`,`thumb`,`cdate`,`intro`,`isactive`) VALUES";
		$sql.="('".$this->Name."', '".$this->GroupID."', '".$this->Code."','".$this->Thumb."','".$this->Cdate."','".$this->Intro."','1')";
	...
	function Update(){
		$sql = "UPDATE tbl_album SET `name`='".$this->Name."',`code`='".$this->Code."',`thumb`='".$this->Thumb."',`intro`='".$this->Intro."',`group_id`='".$this->GroupID."' WHERE id='".$this->ID."'";
	...
	public function getListGallery($strwhere="", $path=""){
		...
		while($rows=$objdata->Fetch_Assoc()){
			...
			?>
			<div class="info-item">
				<img src="<?php echo ROOTHOST.PATH_GALLERY_REVIEW.$path;?>" width="150px">
